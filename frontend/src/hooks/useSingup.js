...
const useSignup = () => {
    ...
    // Kullanıcı formundan gelen bilgileri alır.
    const signup = async ({fullName, username, password, confirmPassword, gender}) => {
        const success = handleInputsErrors({fullName, username, password, confirmPassword, gender});
        if(!success) return;
        setLoading(true);
        try {
            // fetch: belirtilen api endpointine bir POST isteği yapar. İstekler başlıkları ve body'si json formatında gönderilir.
...
function handleInputsErrors({fullName, username, password, confirmPassword, gender}) {
    if (!fullName || !username || !password || !confirmPassword || !gender) {
        toast.error('Pleas fill in all fields!');
        return false;
    }
